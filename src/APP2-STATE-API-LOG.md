...
| Dataset | Count | Notes |
|---------|-------|-------|
| currentImages | 13 | Family moments; Picsum free stock (no API key) |
| growthImages | 25 | Sofia & Liam from newborn 2018 → 2025; tagged by child |
...
| timeline | 12 | Events from 2014 wedding → 2025 |
...
- `QueryClientProvider` wired in `src/index.tsx` with:
  - `retry: 2` for queries, `retry: 1` for mutations
  - `staleTime: 5 minutes` default
  - `refetchOnWindowFocus: false`
  - `ReactQueryDevtools` enabled
- `App.tsx` updated to demonstrate all three hooks working end-to-end
- All components are intended to use hooks only — no direct Zustand calls

---

## API Swap Instructions

To swap mock data for real API endpoints, edit only `src/api/mockData.ts`:

```ts
// Before (mock):
export async function fetchCurrentImages(page = 1, pageSize = 6) {
  await delay(SIMULATED_DELAY);
  return { data: CURRENT_IMAGES.slice(...), page, pageSize, total, totalPages };
}

// After (real API):
export async function fetchCurrentImages(page = 1, pageSize = 6) {
  const res = await fetch(`/api/gallery/current?page=${page}&pageSize=${pageSize}`);
  return res.json(); // Same PaginatedResponse<Image> shape
}
```
...
- 0 errors in `src/stores/`, `src/hooks/`, `src/api/`
- Pre-existing errors in `src/components/` and `src/pages/` (from other parallels) — not in scope
...
dist/static/js/index.js   18.2 kB
dist/static/js/175.js     40.7 kB (includes zustand + react-query)
dist/static/js/lib-react.js  189.7 kB
...
src/
├── api/
│   └── mockData.ts              ← All mock data + fetch functions
├── stores/
│   ├── galleryStore.ts          ← Images, likes, comments
│   ├── uiStore.ts               ← Modal, filters, loading
│   └── familyStore.ts           ← Family data
├── hooks/
│   ├── useCurrentGallery.ts     ← Query: current gallery
│   ├── useGrowthGallery.ts      ← Query: growth gallery w/ filters
│   ├── useFamilyInfo.ts         ← Query: family data
│   ├── useLikeImage.ts          ← Mutation: toggle like (optimistic)
│   ├── useAddComment.ts         ← Mutation: add comment
│   ├── useGalleryFilters.ts     ← Convenience: filter state
│   └── useImageModal.ts         ← Convenience: modal state
└── index.tsx                    ← QueryClientProvider wired here
```
