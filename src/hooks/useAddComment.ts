/**
 * useAddComment.ts
 * React Query mutation to add a comment to an image
 * Syncs new comment into galleryStore immediately on success
 */
...
export function useAddComment() {
  ...
  const mutation = useMutation({
    mutationFn: ({ imageId, text }: AddCommentPayload) =>
      apiAddComment(imageId, text),
