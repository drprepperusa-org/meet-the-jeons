/**
 * useLikeImage.ts
 * React Query mutation to toggle image like
 * Optimistically updates Zustand store before the API response
 */
...
export function useLikeImage() {
  ...
  const mutation = useMutation({
    ...
    onMutate: async (imageId: string) => {
      ...
      setLike(
        imageId,
        previous.liked ? previous.count - 1 : previous.count + 1,
        !previous.liked
      );
    ...
    onError: (_err, _imageId, context) => {
      // Rollback on failure
      if (context) {
        setLike(context.imageId, context.previous.count, context.previous.liked);
      }
    },
    ...
    onSuccess: (response) => {
      if (response.success) {
        ...
        // Invalidate gallery queries so server state syncs
